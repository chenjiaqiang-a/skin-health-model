...
def image_density_train_trans(image, density):
    ...
    top = int((512 - 224) * top_ratio)
    left = int((512 - 224) * left_ratio)
    ...
    density = tf.resize(density, [256, 256])
    top = int((256 - 112) * top_ratio)
    left = int((256 - 112) * left_ratio)
...
def image_density_test_trans(image, density):
    ...
    if density is None:
        density = np.zeros((128, 128), dtype=np.float32)
    density = tf.to_tensor(density)
    density = tf.resize(density, [112, 112])
